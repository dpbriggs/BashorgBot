...
class BashOrgBot(object):
    def __init__(self):
        ...
        self.quote0 = self.quotes[33][0]
        self.ref0 = self.quotes[33][1]
        self.upvotes0 = self.quotes[3][2]
        print(self.quote0)
        print(self.ref0)
        print(self.upvotes0)
    ...
    def submit_post(self, quote, refnum, upvotes):
        quotetext = quote[quote.find('<'):quote.find('\n')]
        if len(quotetext) >= 97:
            quotetext = textwrap.wrap(quotetext, 100)[0]+'...'
        else:
            quotetext = quotetext + '  '
        bodytext = quote + '  \n  [Source](http://bash.org/?%s' % (refnum[1:]+')')
